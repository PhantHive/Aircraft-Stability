...
class AircraftMatrix(FlightData):
    ...
    def calculate_Xu(self, S):
        first_part = (self.cruise_conditions["q_mean"]["value"] *
                      S) / (self.cruise_conditions["m"]["value"] *
                            self.cruise_conditions["V"]["value"])
        second_part = (2 * self.cruise_conditions["C_D_0"]["value"] +
    ...
    def calculate_Zu(self, S):
        first_part = (self.cruise_conditions["q_mean"]["value"] *
                      S) / (self.cruise_conditions["m"]["value"] *
                            self.cruise_conditions["V"]["value"])
        second_part = (2 * self.cruise_conditions["C_L_0"]["value"] +
    ...
    def calculate_Zw(self, S):
        first_part = (self.cruise_conditions["q_mean"]["value"] *
                      S) / (self.cruise_conditions["m"]["value"] *
                            self.cruise_conditions["V"]["value"])
        second_part = (self.cruise_conditions["C_D_0"]["value"] +
                       self.stability_der["C_L_alpha"]["value"])
    ...
    def calculate_Mq(self, wing_mean_chord, S):
        first_part = (self.cruise_conditions["q_mean"]["value"] * S * np.power(
            wing_mean_chord, 2)) / (2 * self.cruise_conditions["Iyy"]["value"]
                                    * self.cruise_conditions["V"]["value"])
    ...
    def set_long_stability_aircraft_matrix(self):
        ...
        self.aircraft_matrix = np.array([
            [
                ...
                np.cos(self.cruise_conditions["theta"]["value"]),
            ],
            [
                self.Zu,
                self.Zw,
                self.cruise_conditions["V"]["value"],
                -self.cruise_conditions["g"]["value"] *
                np.sin(self.cruise_conditions["theta"]["value"]),
            ],
            [
    ...
    def set_damping_ratio(self):
        damping_ratio_sp = -np.real(self.eigenvalues[0]) / np.abs(
            self.eigenvalues[0])
        damping_ratio_p = -np.real(self.eigenvalues[-1]) / np.abs(
            self.eigenvalues[-1])
