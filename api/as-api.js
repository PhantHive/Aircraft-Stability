...
app.post("/process_data", (req, res) => {
  ...
  // replace the double quote at the beginning and end of the string with a
  // single quote
  ...
  process.on("close", (code) => {
    if (code === 0) {
      // If the Python script exits with code 0 (success), return the result to
      // the client
      console.log("RESULT: " + result);
      res.json({ success: true, result: result });
    } else {
      // If the Python script exits with a non-zero code (error), return an
